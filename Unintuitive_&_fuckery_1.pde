...
Mover mover, mover2, mover3, mover4, mover5, mover6;
PVector co1, co2;
int a = 1;
int b = 1;
int c = 1;
...
void draw() {
  ...
  if (d < 2)
  {
    mover.set_location(new PVector(random(0, 1000), -367));
   d++;
  }
  mover.update(true);
  mover.checkEdges();
  ...
  //call a second time from a different location
  if (a < 2)
  {
  mover2.set_location(new PVector(random(0, 1000), 367));
  a++;
  }
  mover2.update(false);
  mover2.checkEdges();
  ...
  if (b < 2)
  {
  mover3.set_location(new PVector(random(0, 1000), -367));
  b++;
  }
  mover3.update(true);
  mover3.checkEdges();
  ...
  if (c < 2)
  {
  mover4.set_location(new PVector(random(0, 1000), 367));
  c++;
  }
  mover4.update(false);
  mover4.checkEdges();
  ...
  if (e < 2)
  {
  mover5.set_location(new PVector(random(0, 1000), -367));
  e++;
  }
  mover5.update(true);
  mover5.checkEdges();
  mover5.display(); 
  mover5.ApplyFriction();
  
  if (f < 2)
  {
  mover6.set_location(new PVector(random(0, 1000), 367));
  f++;
  }
  mover6.update(false);
  mover6.checkEdges();
  mover6.display(); 
  mover6.ApplyFriction();
  ...
  mover.CheckCollision(mover, mover5);
  mover.CheckCollision(mover, mover6);
  mover.CheckCollision(mover2, mover3);
  mover.CheckCollision(mover2, mover4);
  mover.CheckCollision(mover2, mover5);
  mover.CheckCollision(mover2, mover6);
  mover.CheckCollision(mover3, mover4);
  mover.CheckCollision(mover3, mover5);
  mover.CheckCollision(mover3, mover6);
  mover.CheckCollision(mover4, mover5);
  mover.CheckCollision(mover4, mover6);
  mover.CheckCollision(mover5, mover6);
